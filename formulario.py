...
if user == "Actualización datos":
    ...
    try:
            ...
            if info["CUI"].values[0] != cui_prefill:
                ...
                with add_user_f:
                    ...
                    with c_ccc:
                        ...
                        with c_aa:  
                            b_aa = st.button("<")   
                            if (b_aa and st.sessi   on_state.count_ac > 0 and st.session_state.explorar == "Diversificado"):   
                                st.session_state.   count_ac -= 1  
                                pp_lugar_d.markdo   wn("Lugar")    
                                p_lugar_d.info(st   .session_st    ate.lugar_d[st.session_state.count_ac]) 
                                pp_nombre_d.markd   own("Título     obtenido") 
                                p_nombre_d.info(s   t.session_s    tate.titulo _d[st.session_state.count_ac])   
                                pp_estado_d.markd   own("Estado    ")  
                                p_estado_d.info(s   t.session_s    ta  te.e_d[st.session_state.count_ac])    
                                metric_d.metric(l   abel="Págin    a   #", value = st.session_state.count_ac + 1, delta = len(st.session_stat    e.lugar_d), delta_color="off")
                            if (b_aa and st.sessi   on_state.co    un  t_al > 0 and st.session_state.explorar == "Licenciatura"):    
                                st.session_state.   count_al -=     1  
                                pp_lugar_l.markdo   wn("Lugar")    
                                p_lugar_l.info(st   .session_st    ate.lugar_l[st.session_state.count_al]) 
                                pp_nombre_l.markd   own("Título     obtenido") 
                                p_nombre_l.info(s   t.session_s    tate.titulo _l[st.session_state.count_al])   
                                pp_estado_l.markd   own("Estado    ")  
                                p_estado_l.info(s   t.session_s    ta  te.e_l[st.session_state.count_al])    
                                pp_semestre_l.mar   kdown("Seme    st  re")  
                                p_semestre_l.info   (st.session    _s  tate  .e_ll[st.session_state.count_al]) 
                                metric_l.metric(l   abel="Págin    a   #",   value = st.session_state.count_al + 1, delta = len(st.session_stat    e.lugar_l), delta_color="off")
                            if (b_aa and st.sessi   on_state.co    un  t_am   > 0 and st.session_state.explorar == "Maestría/Posgrado"):   
                                st.session_state.   count_am -=     1  
                                pp_lugar_m.markdo   wn("Lugar")    
                                p_lugar_m.info(st   .session_st    ate.lugar_m[st.session_state.count_am]) 
                                pp_nombre_m.markd   own("Título     obtenido") 
                                p_nombre_m.info(s   t.session_s    tate.titulo _m[st.session_state.count_am])   
                                pp_estado_m.markd   own("Estado    ")  
                                p_estado_m.info(s   t.session_s    ta  te.e_m[st.session_state.count_am])    
                                metric_m.metric(l   abel="Págin    a   #", value = st.session_state.count_am + 1, delta = len(st.session_stat    e.lugar_m), delta_color="off")
                            if (b_aa and st.sessi   on_state.co    un  t_acc > 0 and st.session_state.explorar == "Cursos/Diplomados/Certific    aciones"):
                                st.session_state.   count_acc -    =   1 
                                pp_lugar_c.markdo   wn("Lugar")    
                                p_lugar_c.info(st   .session_st    ate.lugar_c[st.session_state.count_acc])    
                                pp_nombre_c.markd   own("Título     obtenido") 
                                p_nombre_c.info(s   t.session_s    tate.titulo _c[st.session_state.count_acc])  
                                pp_estado_c.markd   own("Estado    ")  
                                p_estado_c.info(s   t.session_s    ta  te.e_c[st.session_state.count_acc])   
                                metric_acc.metric   (label="Pág    in  a #", value = st.session_state.count_acc + 1, delta = len(st.session_s    tate.lugar_c), delta_color="off")
                        with c_bb:  
                            b_bb =   st.button("Nuevo")   
                            if (b_  bb and st.session_state.explorar == "Diversificado"): 
                                st  .session_state.n_b_d = True   
                                pp  _lugar_d.empty()  
                                pp  _nombre_d.empty(  ) 
                                pp  _estado_d.empty(  ) 
                                st  .session_state.l  u gar_d.append("") 
                                st  .session_state.t  i tulo_d.append("")    
                                st  .session_state.e  _ d.append("N/A")  
                                pp  _lugar_d.markdow  n ("Lugar")    
                                p_  lugar_d.info(st.  s ession_st    ate.lugar_d[-1])    
                                pp  _nombre_d.markdo  w n("Título     obtenido") 
                                p_  nombre_d.info(st  . session_s    tate.titulo _d[-1])  
                                pp  _nombre_d.markdo  w n("Estado    ")  
                                p_  estado_d.info(st  . session_s    ta  te.e_d[-1])   
                                st  .session_state.c  o unt_ac =     le  n(st.session_state.lugar_d) - 1   
                                me  tric_d.metric(la  b el="Págin    a   #", value = st.session_state.co   unt_ac + 1, delta = len(st.session_stat    e.lugar_d), delta_color="off")
                            elif (  b_bb and st.sess  i on_state.    ex  plorar == "Licenciatura"):    
                                st  .session_state.n  _ b_d = Tru    e   
                                pp  _lugar_l.empty()  
                                pp  _nombre_l.empty(  ) 
                                pp  _estado_l.empty(  ) 
                                pp  _semestre_l.empt  y ()   
                                st  .session_state.l  u gar_l.append("") 
                                st  .session_state.t  i tulo_l.append("" )    
                                st  .session_state.e  _ l.append("N/A")  
                                st  .session_state.e  _ ll.append(0) 
                                pp  _lugar_l.markdow  n ("Lugar")    
                                p_  lugar_l.info(st.  s ession_st    ate.lugar_l[-1])    
                                pp  _nombre_l.markdo  w n("Título     obtenido") 
                                p_  nombre_l.info(st  . session_s    tate.titulo _l[-1])  
                                pp  _estado_l.markdo  w n("Estado    ")  
                                p_  estado_l.info(st  . session_s    ta  te.e_l[-1])   
                                pp  _semestre_l.mark  d own("Seme    st  re")  
                                p_  semestre_l.info(  s t.session    _s  tate  .e_ll[-1])    
                                st  .session_state.c  o unt_al =     le  n(st  .session_state.lugar_l) - 1   
                                me  tric_l.metric(la  b el="Págin    a   #",   value = st.session_state.co   unt_al + 1, delta = len(st.session_stat    e.lugar_l), delta_color="off")
                            elif (  b_bb and st.sess  i on_state.    ex  plor  ar == "Maestría/Posgrado"):   
                                st  .session_state.n  _ b_d = Tru    e   
                                pp  _lugar_m.empty()  
                                pp  _nombre_m.empty(  ) 
                                pp  _estado_m.empty(  ) 
                                st  .session_state.l  u gar_m.append("") 
                                st  .session_state.t  i tulo_m.append("")    
                                st  .session_state.e  _ m.append("N/A")  
                                pp  _lugar_m.markdow  n ("Lugar")    
                                p_  lugar_m.info(st.  s ession_st    ate.lugar_m[-1])    
                                pp  _nombre_m.markdo  w n("Título     obtenido") 
                                p_  nombre_m.info(st  . session_s    tate.titulo _m[-1])  
                                pp  _estado_m.markdo  w n("Estado    ")  
                                p_  estado_m.info(st  . session_s    ta  te.e_m[-1])   
                                st  .session_state.c  o unt_am =     le  n(st.session_state.lugar_m) - 1   
                                me  tric_m.metric(la  b el="Págin    a   #", value = st.session_state.co   unt_am + 1, delta = len(st.session_stat    e.lugar_m), delta_color="off")
                            elif (  b_bb and st.sess  i on_state.    ex  plorar == "Cursos/Diplomados/Ce   rtificaciones"):   
                                st  .session_state.n  _ b_d = Tru    e   
                                pp  _lugar_c.empty()  
                                pp  _nombre_c.empty(  ) 
                            pp_estado_c.empty() 
        ...
        st.sidebar.warning("Correo no registrado")
    #===================================================================
    #===================================================================
